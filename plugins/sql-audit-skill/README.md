...
Then invoke it as `/sql-audit` (see [Usage](#usage)). To hack on it locally, add the marketplace
from a clone instead: `/plugin marketplace add ./` then the same install line.
...
skills/sql-audit/
  ...
  references/credential-manager.md  SQL-auth password storage (Credential Manager)
  references/contexts.md            go-sqlcmd connection contexts
  queries/audit.sql            the read-only audit
scripts/detect-sqlcmd.ps1      locate sqlcmd
scripts/credential.ps1         Windows Credential Manager helper
tests/                         regression fixture (see tests/README.md)
```
